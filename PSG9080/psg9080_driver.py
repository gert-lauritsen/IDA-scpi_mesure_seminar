"""
psg9080_driver.py — Complete Python driver for the Hantek/FeelTech PSG9080

Serial protocol wrapper using **pyserial**.
- Baud: 115200, 8N1
- Commands are ASCII lines beginning with ':' and ending with CRLF ("\r\n").
- 'w##' = write, 'r##' = read. Responses look like ":rXX=payload\r\n".
...
# ---- pyserial sanity check ----
if not hasattr(serial, "Serial"):
    try:
        _serial_path = getattr(serial, "__file__", "<unknown>")
    except Exception:
        _serial_path = "<unknown>"
    raise ImportError(
        "Imported module 'serial' is not pyserial or lacks Serial.\n"
        f"Loaded module path: {_serial_path}\n"
        "Fix: uninstall any 'serial' package and install pyserial, and ensure no local 'serial.py' shadows it.\n"
        "    py -m pip uninstall -y serial\n"
        "    py -m pip uninstall -y pyserial && py -m pip install -U pyserial\n"
    )
...
class Waveform(IntEnum):
    ...
    MULTI_AUDIO = 20
    LORENZ = 21
    # Arbitrary: 101..199 map to Arb 01..99
...
def _freq_to_fields(hz: float) -> Tuple[int, int]:
    """Return (scaled_value, unit_code) using 3 implied decimals per spec.
...
class PSG9080:
    ...
    def open(self) -> None:
        ...
        try:
            ...
            raise PSG9080Error(f"Could not open serial port {self.opts.port}: {e}")
        time.sleep(0.05)
    ...
    def _send(self, line: str) -> None:
        ...
        # Drain quick firmware acks like ":ok\r\n" to avoid polluting next read
        try:
            t0 = time.time()
            while (time.time() - t0) < 0.05 and self._ser.in_waiting:
                resp = self._ser.readline()
                if not resp:
                    break
                if resp.strip().lower() != b":ok":
                    # ignore stray non-ack lines here
                    pass
        except Exception:
            pass
    ...
    def _query(self, line: str) -> str:
        if not self._ser or not self._ser.is_open:
            raise PSG9080Error("Serial port not open. Call open().")
        # Up to 3 attempts in case the device is still processing previous writes
        attempts = 3
        for attempt in range(attempts):
            try:
                try:
                    ...
                    pass
                # tiny pacing before issuing a read
                time.sleep(0.01 if attempt == 0 else 0.03)
                self._send(line)
                deadline = time.time() + max(0.2, float(self.opts.timeout) * (1.5 + 0.5 * attempt))
                ...
                time.sleep(0.05)
        raise PSG9080Error(f"Timeout or malformed response while querying '{line}'")

    # ------------------------------ Commands -------------------------------
    # w10 / r10 — output enable per channel
    ...
    # w11/w12 / r11/r12 — waveform select
    def set_waveform(self, ch: Union[int, Channel], wf: Union[int, Waveform]) -> None:
        ch = _ensure_channel(ch)
        code = int(wf)
        self._send(("w11=" if ch == Channel.CH1 else "w12=") + f"{code}.")
    ...
    # w13/w14 / r13/r14 — frequency (value, unit_code) with 3 implied decimals
    def set_frequency(self, ch: Union[int, Channel], hz: float) -> None:
        ch = _ensure_channel(ch)
        val, unit = _freq_to_fields(hz)
    ...
    def get_frequency(self, ch: Union[int, Channel]) -> float:
        ...
        return _fields_to_freq(value_str, int(unit_str))

    # w15/w16 / r15/r16 — amplitude (Vpp) in mV integer
    ...
    # w17/w18 / r17/r18 — DC offset (raw device code)
    def set_offset_raw(self, ch: Union[int, Channel], code: int) -> None:
        cmd = "w17" if _ensure_channel(ch) == Channel.CH1 else "w18"
        self._send(f"{cmd}={int(code)}.")

    def get_offset_raw(self, ch: Union[int, Channel]) -> int:
        payload = self._query("r17=0." if _ensure_channel(ch) == Channel.CH1 else "r18=0.")
        return int(payload.strip("."))

    # w19/w20 / r19/r20 — duty cycle (hundredths of percent)
    def set_duty(self, ch: Union[int, Channel], percent: float) -> None:
        n = int(round(percent * 100))
        cmd = "w19" if _ensure_channel(ch) == Channel.CH1 else "w20"
        self._send(f"{cmd}={n}.")

    def get_duty(self, ch: Union[int, Channel]) -> float:
        payload = self._query("r19=0." if _ensure_channel(ch) == Channel.CH1 else "r20=0.")
        n = int(payload.strip("."))
        return n / 100.0

    # w21/w22 / r21/r22 — phase (hundredths of degree)
    def set_phase(self, ch: Union[int, Channel], degrees: float) -> None:
        n = int(round(degrees * 100))
        cmd = "w21" if _ensure_channel(ch) == Channel.CH1 else "w22"
        self._send(f"{cmd}={n}.")

    def get_phase(self, ch: Union[int, Channel]) -> float:
        payload = self._query("r21=0." if _ensure_channel(ch) == Channel.CH1 else "r22=0.")
        n = int(payload.strip("."))
        return n / 100.0

    # w24 / r24 — interface/raw (pass-through fields)
    ...
    def memory(self, slot: int, op: str) -> None:
        ...
        self._send(f"w26={int(slot)},{code}.")

    # w27..w33 — UI & preset settings
    ...
    def set_ch1_pm_dev(self, degrees: float) -> None:
        self._send(f"w51={int(round(degrees * 10))}.")

    def set_ch2_pm_dev(self, degrees: float) -> None:
        self._send(f"w52={int(round(degrees * 10))}.")

    # w53..w56 / r53..r56 — pulse width/period
    def set_pulse_width(self, ch: Union[int, Channel], microseconds: float) -> None:
        n = int(round(microseconds * 1000))  # 0.001 us resolution
        cmd = "w53" if _ensure_channel(ch) == Channel.CH1 else "w54"
        self._send(f"{cmd}={n}.")

    def set_pulse_period(self, ch: Union[int, Channel], microseconds: float) -> None:
        n = int(round(microseconds * 100))  # 0.01 us resolution per examples
        cmd = "w55" if _ensure_channel(ch) == Channel.CH1 else "w56"
        self._send(f"{cmd}={n}.")

    def get_pulse_width_us(self, ch: Union[int, Channel]) -> float:
        payload = self._query("r53=0." if _ensure_channel(ch) == Channel.CH1 else "r54=0.")
        return int(payload.strip(".")) / 1000.0

    def get_pulse_period_us(self, ch: Union[int, Channel]) -> float:
        payload = self._query("r55=0." if _ensure_channel(ch) == Channel.CH1 else "r56=0.")
    ...
    def get_burst_count(self, ch: Union[int, Channel]) -> int:
        ...
        a, b = payload.strip(".").split(",")
        return int(a) if _ensure_channel(ch) == Channel.CH1 else int(b)

    # w62 / w63 — measurement configuration & switches
    ...
    def set_sweep_start_freq(self, hz: float) -> None:
        val, _unit = _freq_to_fields(hz)
        self._send(f"w66={val}.")
    ...
    # w74 — trigger per channel (momentary)
    def trigger(self, ch1: bool, ch2: bool) -> None:
        self._send(f"w74={1 if ch1 else 0},{1 if ch2 else 0}.")

    # r81 / r82 — frequency counter (example mapping)
    def get_measurement_frequency_hz(self, high_band: bool = True) -> float:
        payload = self._query("r81=0." if high_band else "r82=0.")
        # Manual shows integer with different scales per range
        return int(payload.strip(".")) / (1.0 if high_band else 1000.0)

    # --------------------------- Convenience -------------------------------
    def configure_basic(
        self,
        ch: Union[int, Channel],
        *,
        waveform: Union[int, Waveform] = Waveform.SINE,
        frequency_hz: float = 1000.0,
        amplitude_vpp: float = 1.0,
        offset_raw: Optional[int] = None,
        duty_percent: Optional[float] = None,
        phase_deg: Optional[float] = None,
        output_on: bool = True,
    ) -> None:
        """Configure common parameters on a channel and optionally enable output.

        Robust version: avoids read-after-write timing by *not* querying output
        state. Instead, it sets outputs explicitly per channel.
        """
        ch = _ensure_channel(ch)
        ...
        # Give the unit a brief moment between heavy config and output toggle
        time.sleep(0.02)
        if ch == Channel.CH1:
            self.set_output(bool(output_on), False)
        else:
            self.set_output(False, bool(output_on))
...
#     opts = OpenOptions(port="COM5", baudrate=115200, timeout=0.8)
#     with PSG9080(opts) as psg:
#         psg.set_output(False, False)
#         psg.configure_basic(1, waveform=Waveform.SINE, frequency_hz=10_000, amplitude_vpp=1.0)
#         psg.set_output(True, False)
#         print("CH1 frequency:", psg.get_frequency(1))
#         psg.set_output(False, False)
